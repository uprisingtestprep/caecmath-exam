/* CAECMATH Exam Simulator, Application Logic */

const ACCESS_CODE  = "CAECMATH9000";
// One session is 200 questions. Timer scaled to the real CAEC pace
// (42 questions / 120 min), so 200 questions gets ~9.5 hours. Finishing early
// (and submitting) is what unlocks the next session.
const EXAM_SECONDS = 34200;
const PASSING_PCT  = 55;
const STORAGE_KEY  = "caecmath_exam_state_v1";
const SESSION_SIZE = 200;
const NUM_SESSIONS = 5;
const CLUSTER_LABEL = "Case";
const DOMAIN_LABELS = {"number_sense_and_operations": "Number Sense and Operations", "patterns_and_relations": "Patterns and Relations", "geometry_and_measurement": "Geometry and Measurement", "data_management_and_probability": "Data Management and Probability"};  // maps domain key -> human-readable label for display
...
let state = {
  ...
  activeSession: null, sessionsDone: {},
};
...
// Split the full bank into NUM_SESSIONS fixed sessions of SESSION_SIZE each,
// in printed (id) order, so every question appears in exactly one session.
function buildSessions(all) {
  const sorted = all.slice().sort((a, b) => (a.id || 0) - (b.id || 0));
  const out = [];
  for (let i = 0; i < NUM_SESSIONS; i++) {
    out.push(sorted.slice(i * SESSION_SIZE, (i + 1) * SESSION_SIZE));
  }
  return out;
}

function sessionRange(n) {
  const a = (n - 1) * SESSION_SIZE + 1;
  const b = Math.min(n * SESSION_SIZE, sessions.length * SESSION_SIZE);
  return `${a}-${b}`;
}

function sessionUnlocked(n) {
  if (n === 1) return true;
  return !!(state.sessionsDone && state.sessionsDone[n - 1]);
}
...
function showSessionSelect() {
  ...
  document.getElementById("results-screen").style.display = "none";
  renderSessionSelect();
  document.getElementById("session-select").style.display = "flex";
}

function renderSessionSelect() {
  const grid = document.getElementById("session-grid");
  grid.innerHTML = "";
  const inProgress = state.activeSession && !state.submitted
    && Object.keys(state.answers).length > 0;
  for (let n = 1; n <= NUM_SESSIONS; n++) {
    const unlocked = sessionUnlocked(n);
    const done = !!(state.sessionsDone && state.sessionsDone[n]);
    ...
    else if (inProgress && state.activeSession === n) status = "In progress";
    else status = "Ready";
    div.innerHTML = `<div class="session-num">${n}</div>`
      + `<div class="session-name">Session ${n}</div>`
      + `<div class="session-range">Questions ${sessionRange(n)}</div>`
      + `<div class="session-status">${status}</div>`;
    div.addEventListener("click", () => {
      if (unlocked) startSession(n);
      else alert(`Session ${n} is locked. Finish Session ${n - 1} to unlock it.`);
    });
    grid.appendChild(div);
  }
}

// ── exam start (per session) ──────────────────────────────────────────────────
function startSession(n) {
  if (n < 1 || n > NUM_SESSIONS || !sessionUnlocked(n)) {
    alert(`Session ${n} is locked.`);
    return;
  }
  questions = sessions[n - 1].slice();
  const resume = state.activeSession === n && !state.submitted
    && Object.keys(state.answers).length > 0;
  if (!resume) {
    state = {
      phase: "app", answers: {}, flags: {}, current: 1,
      timeLeft: EXAM_SECONDS, submitted: false, startTime: null,
      activeSession: n, sessionsDone: state.sessionsDone || {},
    };
  } else {
    state.activeSession = n;
  }
  document.getElementById("session-select").style.display = "none";
  document.getElementById("results-screen").style.display = "none";
...
// Bind every control exactly once (startSession can run many times).
function bindControls() {
  ...
  document.getElementById("res-review-btn").addEventListener("click", showReview);
  document.getElementById("res-sessions-btn").addEventListener("click", () => showSessionSelect());
  document.getElementById("res-restart-btn").addEventListener("click", retakeSession);
...
function renderQuestion() {
  ...
  document.getElementById("q-counter").textContent =
    `Session ${state.activeSession || 1} | Question ${state.current} of ${questions.length}`;
  ...
  if (q.type === "fill_in_blank") {
    ...
    input.value = typeof chosen === "string" ? chosen : "";
...
// ── fill-in-the-blank (direct entry) support ────────────────────────────────
...
// Normalizes a typed answer and the keyed correct_value: if both parse as
// floats, compare numerically (so "45", "45.0" and "45.00" all match); else
// fall back to a case-insensitive exact string match.
...
function submitExam() {
  ...
  if (state.activeSession && !state.sessionsDone[state.activeSession]) {
    state.sessionsDone[state.activeSession] = true;
  }
  saveState();
  showResults();
}
...
function showResults() {
  ...
  const next = (state.activeSession || 1) + 1;
  const note = document.getElementById("res-unlock-note");
  if (note) {
    note.textContent = next <= NUM_SESSIONS
      ? `Session ${state.activeSession} complete. Session ${next} is now unlocked.`
      : "All sessions complete. You can review or retake any session.";
  }
